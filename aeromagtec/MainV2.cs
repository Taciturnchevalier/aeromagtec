...
namespace aeromagtec
{
    public partial class MainV2 : Form
    {
        ...
        public MainV2()
        {
            ...
            //aeromagtec.Utilities.Tracking.cid = new Guid(Settings.Instance["guid"].ToString());
            ...
            try
            {
                if (!Directory.Exists(Settings.Instance.DataDir))
                    Directory.CreateDirectory(Settings.Instance.DataDir);
            }
            catch (Exception ex) { log.Error(ex); }
        ...
        public void changelanguage(CultureInfo ci)
        {
            ...
            if (ci != null && !Thread.CurrentThread.CurrentUICulture.Equals(ci))
            {
                ...
                HashSet<Control> views = new HashSet<Control> { this, FlightData };
        ...
        public void doConnect(LinkInterface comPort, string portname, string baud)
        {
                ...
                try
                {
                    ...
                    lock (this)
                    {
                        ...
                        var rlog = Settings.Instance.LogDir + Path.DirectorySeparatorChar +
                                   dt + ".rlog";
                        //var datadb = Settings.Instance.DataDir + Path.DirectorySeparatorChar +
                        //dt + ".fdb";

                        var datadb = AppDomain.CurrentDomain.BaseDirectory.Replace("bin\\Debug\\", "") + "DB\\TEST.FDB";
                        ...
                        while (File.Exists(tlog))
                        {
                            ...
                            rlog = Settings.Instance.LogDir + Path.DirectorySeparatorChar +
                                   dt + ".rlog";
                            datadb = Settings.Instance.DataDir + Path.DirectorySeparatorChar +
                                  dt + ".fdb";
                        }
                        ...
                        CreateEmDataBase(datadb, comPort);

                        log.Info("creating logfile " + dt + ".tlog");
                        log.Info("creating database" + datadb + ".fdb");
                    }
                }
        ...
        private void CreateEmDataBase(string pathDb, LinkInterface comPort)
        {
            ...
            using (comPort.connection = new FbConnection(Csb.ToString()))
            {
                comPort.connection.Open();
                using (FbCommand createTable = comPort.connection.CreateCommand())
                {
                    createTable.CommandText = "create table FLYDATA (id bigint, CheckFlg int, Count int)";
        ...
        public void updateLayout(object sender, EventArgs e)
        {
            ...
            aeromagtec.Controls.BackstageView.BackstageView.Advanced = DisplayConfiguration.isAdvancedMode;
            ...
            if (MainV2.instance.FlightData != null)
            {
                ...
                if (DisplayConfiguration.displayQuickTab && !t.TabPages.Contains(FlightData.quickview))
                {
                    t.TabPages.Add(FlightData.quickview);
                }
                else if (!DisplayConfiguration.displayQuickTab && t.TabPages.Contains(FlightData.recordSet))
                {
                    t.TabPages.Remove(FlightData.recordSet);
                }
                if (DisplayConfiguration.displayPreFlightTab && !t.TabPages.Contains(FlightData.tabStatus))
                {
                    t.TabPages.Add(FlightData.tabStatus);
                }
                else if (!DisplayConfiguration.displayPreFlightTab && t.TabPages.Contains(FlightData.tabLogread))
                {
                    t.TabPages.Remove(FlightData.tabLogread);
                }
                if (DisplayConfiguration.displayAdvActionsTab && !t.TabPages.Contains(FlightData.tabDatacheck))
                {
                    t.TabPages.Add(FlightData.tabDatacheck);
                }
                else if (!DisplayConfiguration.displayAdvActionsTab && t.TabPages.Contains(FlightData.tabDatafliter))
                {
                    t.TabPages.Remove(FlightData.tabDatafliter);
                }
        ...
        protected override void OnLoad(EventArgs e)
        {
            ...
            //aeromagtec.Utilities.Tracking.AddTiming("AppLoad", "Load Time",
            ...
            if (winXp)
            {
                ...
                System.Configuration.ConfigurationManager.AppSettings["UpdateLocationVersion"] =
                    "http://firmware.ardupilot.org/aeromagtec/xp/";
                ...
                System.Configuration.ConfigurationManager.AppSettings["UpdateLocationMD5"] =
                    "http://firmware.ardupilot.org/aeromagtec/xp/checksums.txt";
            ...
            try
            {
                ...
                {
                    //aeromagtec.Utilities.Update.dobeta = true;
                    System.Threading.ThreadPool.QueueUserWorkItem(checkupdate);
                }
            }
            catch (Exception ex)
        ...
        protected override bool ProcessCmdKey(ref Message msg, Keys keyData)
        {
            ...
            //{
            //    aeromagtec.GenOTP otp = new aeromagtec.GenOTP();
        ...
        private void adsb_UpdatePlanePosition(object sender, EventArgs e)
        {
            lock (adsblock)
            {
                var adsb = ((aeromagtec.Utilities.adsb.PointLatLngAltHdg)sender);
        ...
        private void AutoHideMenu(bool hide)
        {
            ...
            if (!hide)
            {
                ...
                connectionControl1.MouseLeave -= MainMenu_MouseLeave;
                this.ResumeLayout();
            }
            else
            {
        ...
        private void BGCreateMaps(object state)
        {
            ...
            try
            {
                ...
                aeromagtec.Log.LogSort.SortLogs(Directory.GetFiles(Settings.Instance.LogDir, "*.rlog"));
            }
            catch (Exception ex)
            {
                log.Error(ex);
        ...
        private void checkupdate(object stuff)
        {
            ...
            try
            {
                //aeromagtec.Utilities.Update.CheckForUpdate();
            }
            catch (Exception ex)
            {
                log.Error("Update check failed", ex);
        ...
        private void cmb_sysid_Click(object sender, EventArgs e)
        {
        ...
        private void connectionListToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            if (File.Exists(openFileDialog.FileName))
            {
                ...
                foreach (var line in lines)
                {
                    try
                    {
                        LinkInterface mav = new LinkInterface();
        ...
        private void KIndex_KIndex(object sender, EventArgs e)
        {
        ...
        private void MainMenu_ItemClicked(object sender, ToolStripItemClickedEventArgs e)
        {
            ...
            //MainMenu.BackgroundImage = aeromagtec.Properties.Resources.bgdark;
        }
        ...
        private void MenuArduPilot_Click(object sender, EventArgs e)
        {
            try
            {
                System.Diagnostics.Process.Start("http://aircas.ac.cn");
            }
            catch
            {
                CustomMessageBox.Show("Failed to open url http://aircas.ac.cn");
            }
        }
